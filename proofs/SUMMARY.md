...
| **P8** Observability quality | Would a SigNoz eng say "they get OTel"? | checklist all ✓ | **EMIT-SIDE PASS (#16)** — graded against REAL captured spans/metrics: correct semconv (`gen_ai.provider.name` not `system`; `input/output_tokens` no `total_tokens`; `chat {model}`/`invoke_agent {agent}`; CLIENT/INTERNAL kinds; status UNSET on success, Error+`error.type` on 5xx; low-cardinality `argus_*` labels). Fixed 2 tells: added `service.instance.id` + `deployment.environment.name`; Ada Simple→Batch. **2 SigNoz-UI checks (service map, metric→trace) pending the demo box** — not fake-ticked. | 🟡 |
